"""
HaemoRL v4.0 — 1500 patients ALL with diseases + AI Chatbot
"""
...
HF_REPO_ID=os.getenv("HF_REPO_ID","")  # thestoicrohit/haemorl-data
...
DISEASE_DB={
    ...
    "infectious":{"diseases":["Sepsis Multi-Organ Dysfunction SOFA >10","Infective Endocarditis Surgical Emergency","Meningococcal Septicaemia with DIC","Cerebral Malaria Severe with Coma","Necrotising Fasciitis Type 1","Invasive Aspergillosis Disseminated","Mucormycosis Rhino-Orbital-Cerebral","Gram-Negative Bacteraemia ESBL","C.Difficile Fulminant Colitis","Legionella Pneumonia Severe","Staphylococcal Toxic Shock"],"treatment":"Emergency Blood Transfusion + Antibiotics","organ":"Multi-Organ","need_type":"blood","symptoms":["High fever >40 C","Hypotension","Tachycardia","Altered consciousness","Petechiae","MOF"],"medications":["Meropenem","Vancomycin","Antifungals","Vasopressors","IVIG","Corticosteroids"],"lab_markers":{"WBC":"very high or low","CRP":">300","Procalcitonin":">10","Lactate":"high"}},
...
MN=["Arjun","Rahul","Vikram","Siddharth","Karan","Rohan","Aditya","Mohammed","James","Carlos","Zheng","Taro","Kwame","Ivan","Luca","Sven","Yusuf","Omar","Diego","Ramesh","Vijay","Santosh","Balaji","Deepak","Suresh","Pradeep","Rajesh","Amit","Nikhil","Rohit","Gaurav","Manish","Rajan","Subhash","Dinesh","Naresh","Harish","Girish","Mahesh","Lokesh","Rakesh","Umesh","Ritesh","Hitesh","Jitesh","Paresh","Bhushan","Chetan","Kaustubh","Yogesh"]
FN=["Priya","Ananya","Sneha","Pooja","Divya","Meera","Kavya","Isha","Fatima","Sarah","Emily","Maria","Sofia","Yuki","Amara","Emma","Olivia","Luna","Ingrid","Zainab","Mei","Rekha","Sunita","Geeta","Noa","Luisa","Elena","Chioma","Astrid","Naomi","Kavitha","Lakshmi","Padma","Savitha","Pushpa","Usha","Sudha","Vijaya","Manjula","Kamala","Leela","Seema","Reema","Veena","Neena","Beena","Teena","Meena","Heena","Sheena"]
...
def _db_payload():
    return {"patients":list(DB.patients.values()),"donors":list(DB.donors.values()),"blood_bank":DB.blood_bank,"allocations":list(DB.allocations.values()),"alerts":DB.alerts[-200:],"analytics":DB.analytics[-100:],"llm_log":DB.llm_log[-100:],"chat_history":DB.chat_history[-200:],"counters":{"pctr":DB._pctr,"dctr":DB._dctr,"actr":DB._actr,"alctr":DB._alctr},"ep_cum":DB.ep_cum,"ep_expired":DB.ep_expired,"saved_at":_ts()}

def save_db():
    """Save to local file + HuggingFace Dataset repo (permanent storage)."""
    try:
        ...
        if HF_REPO_ID and HF_TOKEN:
            try:
                ...
                # HF Hub: upload file via PUT to the raw upload endpoint
                url=f"https://huggingface.co/api/datasets/{HF_REPO_ID}/upload/{HF_DATASET_FILE}"
                ...
                print("[SAVE] Pushed to HF Dataset OK",flush=True)
            except Exception as he: print(f"[SAVE] HF push failed: {he}",flush=True)
    except Exception as e: print(f"[SAVE] {e}",flush=True)

def load_db():
    """Load from HF Dataset first (permanent), fall back to local file."""
    # Try HF Dataset first
    if HF_REPO_ID and HF_TOKEN:
        try:
            import urllib.request
            url=f"https://huggingface.co/datasets/{HF_REPO_ID}/resolve/main/{HF_DATASET_FILE}"
            req=urllib.request.Request(url,headers={"Authorization":f"Bearer {HF_TOKEN}"},method="GET")
            resp=urllib.request.urlopen(req,timeout=20)
            data=json.loads(resp.read().decode())
            _restore_db(data)
            print(f"[LOAD] From HF Dataset: {len(DB.patients)} patients, {len(DB.donors)} donors",flush=True)
            return True
        except Exception as e: print(f"[LOAD] HF Dataset failed: {e}, trying local...",flush=True)
    # Fall back to local file
    if not SAVE_FILE.exists(): return False
    try:
        data=json.loads(SAVE_FILE.read_text())
        _restore_db(data)
        print(f"[LOAD] From local: {len(DB.patients)} patients, {len(DB.donors)} donors",flush=True)
        return True
    except Exception as e: print(f"[LOAD] Failed: {e}",flush=True); return False
...
def _seed():
    ...
    for i,h in enumerate(HOSPITALS):
        load=_ri(48,95); DB.hospitals[h]={"id":f"H-{str(i+1).zfill(3)}","name":h,"load_pct":load,"beds_total":_ri(300,3000),"beds_available":_ri(15,400),"icu_beds":_ri(40,300),"icu_available":_ri(3,60),"has_transplant":True,"coords":HCOORDS[i]}
    ...
    for bt in BLOOD_TYPES:
        DB.blood_bank[bt]={"blood_type":bt,"units":_ri(25,160),"component":_r(["Whole Blood","Packed RBC","Platelets","Fresh Frozen Plasma"]),"expiry_days":_ri(5,42)}
    cats=list(DISEASE_DB.keys()); per_cat=SEED_COUNT//len(cats); remainder=SEED_COUNT%len(cats)
    print(f"[SEED] Generating {SEED_COUNT} patients across {len(cats)} disease categories...",flush=True)
    for ci,cat in enumerate(cats):
        count=per_cat+(1 if ci<remainder else 0)
        for _ in range(count):
            p=_make_pt(cat); DB.patients[p["id"]]=p
        print(f"[SEED] {cat}: {count} patients done",flush=True)
    for i in range(25):
        d=_make_donor(i); DB.donors[d["id"]]=d
    for p in [x for x in DB.patients.values() if x["urgency"]=="critical"][:25]:
        DB.alerts.append({"id":_nalid(),"severity":"critical","title":f"Critical: {p['name']}","message":f"{p['disease']} - {p.get('ischaemia_h',0):.1f}h remaining - {p['hospital']}","patient_id":p["id"],"created_at":_ts(),"resolved":False})
    DB._seeded=True; save_db()
    print(f"[SEED] DONE - {len(DB.patients)} patients, {len(DB.donors)} donors saved",flush=True)

def _reward(patient,donor,hospital_name,expired=0,crit_remaining=0,action_type="match_organ"):
    if action_type=="skip":
        v=-0.30 if crit_remaining>0 else 0.0; return {"value":v,"hla_score":0.0,"breakdown":{"skip":v},"explanation":f"Skip ({crit_remaining} critical waiting)"}
    if not patient: return {"value":-0.50,"hla_score":0.0,"breakdown":{"invalid":-0.50},"explanation":"Patient not found"}
    if not donor: return {"value":-0.40,"hla_score":0.0,"breakdown":{"no_donor":-0.40},"explanation":"No donor"}
    r=0; bd={}
    bok=_blood_ok(donor.get("blood_type",""),patient.get("blood_type","")); bd["blood"]=0.25 if bok else -0.15; r+=bd["blood"]
    hsc=_hla(patient.get("hla",{}),donor.get("hla",{})); bd["hla"]=round(hsc*0.35,4); r+=bd["hla"]
    ih=patient.get("ischaemia_h",24); bd["isch"]=(-0.30 if ih<=0 else 0.20 if ih<2 else 0.12 if ih<6 else 0.06 if ih<12 else 0.02 if ih<24 else -0.04); r+=bd["isch"]
    bd["paed"]=0.15 if patient.get("is_paediatric") else 0.0; r+=bd["paed"]
    load=DB.hospitals.get(hospital_name,{}).get("load_pct",70); bd["hosp"]=(-0.20 if load>92 else -0.12 if load>85 else -0.05 if load>75 else 0.05 if load<55 else 0.0); r+=bd["hosp"]
    surv=max(0,1-(patient.get("age",50)/120))
    if patient.get("meld_score") and patient["meld_score"]>30: surv-=0.1
    bd["surv"]=round(surv*0.15,4); r+=bd["surv"]
    if expired>0: bd["expiry"]=round(-0.05*expired,3); r+=bd["expiry"]
    return {"value":round(max(-1.0,min(1.0,r)),3),"hla_score":round(hsc,3),"breakdown":bd,"explanation":f"blood={'OK' if bok else 'BAD'} HLA={int(hsc*100)}% isch={ih:.1f}h paed={'YES' if patient.get('is_paediatric') else 'NO'}"}

def _grade_single():
    done=[a for a in DB.allocations.values() if a["status"] in("complete","active","pending")]
    if not done: return {"task_id":"single_match","score":0.0,"passed":False,"details":"No allocations","metrics":{}}
    ...
    return {"task_id":"single_match","score":s,"passed":s>=0.25,"details":f"hla={int(best.get('hla_score',0)*100)}% score={s}","metrics":{"n":len(done)}}

def _grade_batch():
    done=[a for a in DB.allocations.values() if a["status"] in("complete","active","pending")]; n=len(done)
    if n==0: return {"task_id":"batch_allocation","score":0.0,"passed":False,"details":"No allocations","metrics":{}}
    ...
    return {"task_id":"batch_allocation","score":max(0.0,s),"passed":s>=0.30,"details":f"n={n} avghla={int(avghla*100)}% score={s}","metrics":{"matches":n,"avg_hla":round(avghla,2)}}
...
def _grade_crisis():
    ...
    return {"task_id":"crisis_routing","score":max(0.0,s),"passed":s>=0.20,"details":f"steps={step} cum={cum:.2f} crit={crit_m} exp={exp} score={s}","metrics":{"steps":step,"cum":round(cum,3),"crit_matched":crit_m}}
...
CHAT_SYSTEM="""You are HaemoBot - the AI medical assistant for HaemoRL, India's organ allocation system.
You help with: diseases, treatments, HLA compatibility, lab values, ischaemia, patient prioritisation, India transplant crisis, RL reward function.
Be helpful, accurate, empathetic. Use correct medical terminology. Keep responses clear and concise."""

async def llm_chat(messages,context=""):
    if not HF_TOKEN:
        return ("HaemoBot needs HF_TOKEN to activate. Add it as a Secret in Space Settings.\n\n"
                "Meanwhile: All 1500 patients have specific diseases across 13 categories with HLA typing, ischaemia clocks, lab values, symptoms, and medications.")
    for attempt in range(2):
        try:
            from openai import AsyncOpenAI
            client=AsyncOpenAI(base_url=API_BASE_URL,api_key=HF_TOKEN)
            full_sys=CHAT_SYSTEM+(f"\n\nCurrent system: {context}" if context else "")
            timeout=25.0 if attempt==0 else 40.0
            resp=await asyncio.wait_for(
                client.chat.completions.create(model=MODEL_NAME,messages=[{"role":"system","content":full_sys}]+messages[-10:],temperature=0.4,max_tokens=600),
                timeout=timeout)
            return resp.choices[0].message.content
        except asyncio.TimeoutError:
            if attempt==0: logger.warning("LLM chat timeout, retrying..."); continue
            return "The AI is taking too long. Please try again in a moment."
        except Exception as e:
            logger.error(f"LLM chat error attempt {attempt+1}: {e}")
            if attempt==0: continue
            return f"AI temporarily unavailable. The rule-based system is still running allocations."
...
app=FastAPI(title="HaemoRL",version="4.0.0",description="RL-Based Smart Organ Allocation with AI Chatbot",docs_url="/docs")
...
async def global_exception_handler(request:Request, exc:Exception):
    ...
    return JSONResponse(status_code=500, content={"error":"Internal server error","detail":str(exc)[:200],"path":str(request.url.path)})
...
async def startup():
    logger.info("HaemoRL v4.0 starting up...")
    try:
        loaded = load_db()
        if not loaded:
            logger.info("No saved data found, seeding fresh database...")
            _seed()
        else:
            logger.info(f"Loaded {len(DB.patients)} patients from storage")
        # Always ensure hospitals/routes exist
        if not DB.hospitals:
            logger.info("Seeding hospital network...")
            for i,h in enumerate(HOSPITALS):
                DB.hospitals[h]={"id":f"H-{str(i+1).zfill(3)}","name":h,"load_pct":_ri(48,95),"beds_total":_ri(300,3000),"beds_available":_ri(15,400),"icu_beds":_ri(40,300),"icu_available":_ri(3,60),"has_transplant":True,"coords":HCOORDS[i]}
        if not DB.routes:
            for i in range(len(HOSPITALS)):
                for j in range(i+1,len(HOSPITALS)):
                    if random.random()<0.38:
        ...
        if not DB.blood_bank:
            for bt in BLOOD_TYPES:
                DB.blood_bank[bt]={"blood_type":bt,"units":_ri(25,160),"component":"Whole Blood","expiry_days":_ri(5,42)}
    except Exception as e:
        logger.critical(f"Startup error: {e} — attempting recovery")
        traceback.print_exc()
        try: _seed()
        except: pass
    # Start background task with restart-on-crash wrapper
    asyncio.create_task(_bg_wrapper())
    logger.info(f"HaemoRL ready — {len(DB.patients)} patients, {len(DB.donors)} donors, LLM={'enabled' if HF_TOKEN else 'rule-based'}")

async def _bg_wrapper():
    """Restart background task if it crashes."""
    while True:
        try:
            await _bg()
        except Exception as e:
            logger.error(f"Background task crashed: {e}, restarting in 30s...")
            await asyncio.sleep(30)

async def _bg():
    """Background task with full error isolation — never crashes the server."""
    ...
    while True:
        try:
            ...
            try:
                ...
                logger.warning(f"Ischaemia tick error: {e}")
            # Analytics snapshot
            try:
                ...
                if len(DB.analytics) > 500:  # Trim to prevent memory bloat
                    DB.analytics = DB.analytics[-200:]
            except Exception as e:
                logger.warning(f"Analytics error: {e}")
            # Memory management - trim old data
            try:
                ...
                if len(DB.allocations) > 1000:
                    # Keep most recent 500 allocations
                    alloc_list = sorted(DB.allocations.values(), key=lambda a:a.get("created_at",""))
                    DB.allocations = {a["id"]:a for a in alloc_list[-500:]}
            except Exception as e:
                logger.warning(f"Memory trim error: {e}")
            # Save
            try:
                save_db()
            except Exception as e:
                logger.error(f"Save failed in bg task: {e}")
            # Broadcast
            try:
            ...
            logger.error(f"Background task error #{consecutive_errors}: {e}")
            if consecutive_errors > 10:
                logger.critical("Too many consecutive bg errors, sleeping 5 min")
...
async def ui():
    ...
    return HTMLResponse("<h1>HaemoRL - Upload index.html alongside app.py</h1>")

@app.get("/health")
def health():
    import gc
    pts = list(DB.patients.values())
    crit = sum(1 for p in pts if p.get("urgency")=="critical")
    return {
        "status":"healthy",
        "ts":_ts(),
        "version":"4.0.0",
        ...
        "blood_bank_types":len(DB.blood_bank),
        "alerts_active":sum(1 for a in DB.alerts if not a.get("resolved")),
        "llm_decisions":len(DB.llm_log),
        "chat_messages":len(DB.chat_history),
        "uptime_ok":True,
    }
...
async def api_dispense(body:dict,bg:BackgroundTasks):
    ...
    DB.blood_bank[bt]["units"]-=units; save_db(); bg.add_task(wsm.broadcast,{"event":"blood_dispensed","blood_type":bt,"units":units,"ts":_ts()})
...
async def api_commit(body:dict,bg:BackgroundTasks):
    ...
    p["is_allocated"]=True; p["allocation_id"]=aid; save_db(); bg.add_task(wsm.broadcast,{"event":"allocation_created","allocation_id":aid,"score":rew["value"],"ts":_ts()})
    return alloc
...
async def api_run_episode(body:dict):
    ...
    for i in range(max_s):
        ...
        steps.append({"step":i+1,"action":f"{p.get('name','?')[:12]}>{d.get('name','?')[:12]}","detail":f"Blood:{p.get('blood_type')}/{d.get('blood_type')} HLA:{int(rew['hla_score']*100)}% Isch:{p.get('ischaemia_h',0):.1f}h Disease:{p.get('disease','?')[:20]}"+(" PAED" if p.get("is_paediatric") else ""),"reward":rew["value"],"cum":round(cum,3),"hla_score":rew["hla_score"],"is_paediatric":p.get("is_paediatric",False)})
...
async def api_chat(body:ChatMsg,bg:BackgroundTasks,request:Request):
    ...
    if not _rate_check(f"chat_{request.client.host if request.client else 'anon'}", 20):
        raise HTTPException(429,"Too many requests. Please wait a moment.")
    pts=list(DB.patients.values()); crit=[p for p in pts if p.get("urgency")=="critical"]
    ctx=f"Patients: {len(pts)} | Critical: {len(crit)} | Paed critical: {sum(1 for p in crit if p.get('is_paediatric'))} | Allocations: {len(DB.allocations)} | Donors available: {sum(1 for d in DB.donors.values() if d.get('available'))} | Expired organs: {DB.ep_expired} | Categories: {', '.join(DISEASE_DB.keys())} | Most urgent: {(crit[0].get('name','?')+' ('+crit[0].get('disease','')+', '+str(round(crit[0].get('ischaemia_h',0),1))+'h)') if crit else 'None'}"
    session_msgs=[]
...
async def api_clear_chat(bg:BackgroundTasks):
    DB.chat_history.clear(); save_db(); bg.add_task(wsm.broadcast,{"event":"chat_cleared","ts":_ts()})
    return {"cleared":True}
...
def api_llm_models():
    return {"models":[{"id":"Qwen/Qwen2.5-72B-Instruct","name":"Qwen 2.5 72B"},{"id":"meta-llama/Llama-3.1-70B-Instruct","name":"Llama 3.1 70B"},{"id":"mistralai/Mistral-7B-Instruct-v0.3","name":"Mistral 7B"}],"current":MODEL_NAME if HF_TOKEN else "rule_based","has_key":bool(HF_TOKEN)}
...
def _rate_check(key:str, max_per_minute:int=30) -> bool:
    now=time.time(); window=60
    if key not in _rate_limits: _rate_limits[key]=[]
    _rate_limits[key]=[t for t in _rate_limits[key] if now-t<window]
    if len(_rate_limits[key])>=max_per_minute: return False
...
def api_status():
    ...
    return {
        "ok":True,"ts":_ts(),"version":"4.0.0",
        "patients":len(DB.patients),"donors":len(DB.donors),
        "allocations":len(DB.allocations),"alerts_unresolved":sum(1 for a in DB.alerts if not a.get("resolved")),
        "llm_enabled":bool(HF_TOKEN),"hf_dataset_connected":bool(HF_REPO_ID),
        "save_exists":SAVE_FILE.exists(),
        "memory_patients":len(DB.patients),"memory_analytics":len(DB.analytics),
        "ep_step":DB.ep_step,"ep_expired":DB.ep_expired,"ep_cum_reward":round(DB.ep_cum,3),
    }
