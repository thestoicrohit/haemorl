...
SEED_COUNT=int(os.getenv("SEED_PATIENTS","700"))
...
def grade():
    ...
    # Clamp all scores strictly between 0 and 1 (exclusive) as required by validator
    for k in r:
    ...
    return {"episode_id":DB.ep_id,"grader_results":r,"mean_score":mean,"all_passed":all(v["passed"] for v in r.values()),"graded_at":_ts()}
...
@app.get("/api/rl/smart-decide")
async def api_smart_decide():
    """Smart RL agent decision using full reward function scoring."""
    pts=list(DB.patients.values())
    dns=list(DB.donors.values())
    hos=list(DB.hospitals.values())
    crit=[p for p in pts if p.get("urgency")=="critical" and not p.get("is_allocated")]
    avd=[d for d in dns if d.get("available")]
    if not crit or not avd:
        return {"decision":None,"reason":"No critical patients or donors"}
    best_score=-999; best_p=None; best_d=None
    for p in crit[:8]:
        for d in avd[:8]:
            rew=_reward(p,d,hos[0]["name"] if hos else "AIIMS New Delhi",DB.ep_expired,len(crit))
            if rew["value"]>best_score:
                best_score=rew["value"]; best_p=p; best_d=d; best_rew=rew
    if not best_p: return {"decision":None,"reason":"No valid pair found"}
    bh=min(hos,key=lambda h:h.get("load_pct",100)) if hos else {"name":"AIIMS New Delhi"}
    return {"decision":{"patient_id":best_p["id"],"donor_id":best_d["id"],"hospital":bh["name"],"action_type":"match_organ"},"predicted_reward":best_score,"breakdown":best_rew["breakdown"],"explanation":best_rew["explanation"],"patient":{"name":best_p.get("name"),"disease":best_p.get("disease"),"urgency":best_p.get("urgency"),"ischaemia_h":best_p.get("ischaemia_h")},"donor":{"name":best_d.get("name"),"blood_type":best_d.get("blood_type")}}
