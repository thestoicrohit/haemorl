...
import asyncio,json,math,os,random,uuid
...
async def llm_chat(messages,context=""):
    ...
    try:
        ...
        resp=await asyncio.wait_for(client.chat.completions.create(model=MODEL_NAME,messages=[{"role":"system","content":full_sys}]+messages,temperature=0.4,max_tokens=800),timeout=25.0)
        return resp.choices[0].message.content
    except asyncio.TimeoutError: return "Response timed out. Please try again."
    except Exception as e: return f"Error: {str(e)[:150]}"
...
async def _bg():
    while True:
        ...
        DB.analytics.append({"ts":_ts(),"total":len(DB.patients),"critical":sum(1 for p in DB.patients.values() if p.get("urgency")=="critical"),"allocations":len(DB.allocations),"expired":DB.ep_expired})
        save_db(); await wsm.broadcast({"event":"tick","ts":_ts(),"critical":sum(1 for p in DB.patients.values() if p.get("urgency")=="critical"),"expired":DB.ep_expired})
...
@app.get("/health")
def health():
    return {"status":"healthy","ts":_ts(),"patients":len(DB.patients),"donors":len(DB.donors),"allocations":len(DB.allocations),"llm_enabled":bool(HF_TOKEN),"model":MODEL_NAME if HF_TOKEN else "rule_based","persistent_storage":SAVE_FILE.exists(),"chatbot_enabled":bool(HF_TOKEN),"disease_categories":len(DISEASE_DB),"total_diseases":sum(len(v["diseases"]) for v in DISEASE_DB.values())}
...
@app.post("/api/chat")
async def api_chat(body:ChatMsg,bg:BackgroundTasks):
    pts=list(DB.patients.values()); crit=[p for p in pts if p.get("urgency")=="critical"]
...
async def websocket_endpoint(websocket:WebSocket):
    ...
    try:
        ...
        while True:
            ...
            await websocket.send_json({"event":"heartbeat","ts":_ts(),"critical":sum(1 for p in DB.patients.values() if p.get("urgency")=="critical"),"allocations":len(DB.allocations),"chat_messages":len(DB.chat_history)})
